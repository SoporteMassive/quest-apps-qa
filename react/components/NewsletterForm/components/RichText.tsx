...
function RichText({ text = '' }: Props) {
  ...
  if (!isMounted) {
    ...
    renderer.current.html = (html: any) => escapeHtml(html)
    renderer.current.table = (header: any, body: any) => `
    ...
    renderer.current.tablerow = (content: any) => {
      return `<tr class=${handles.tableTr}>\n${content}</tr>\n`
    }

    renderer.current.tablecell = (
      content: string,
      flags: { header: any; align: any }
    ) => {
